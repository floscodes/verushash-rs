use haraka::haraka512_5;

mod haraka;

pub fn verus_hash(data: &[u8]) -> [u8; 32] {
    let mut buf = [0u8; 128];
    let mut buf_ptr: isize = 0;
    let mut next_offset: isize = 64;
    let mut buf_ptr2: isize = buf_ptr + next_offset;
    ...
    while pos < len {
        let dst = (buf_ptr + 32) as usize;

        if len - pos >= 32 {
            buf[dst..dst + 32].copy_from_slice(&data[pos..pos + 32]);
        } else {
            let r = len - pos;
            buf[dst..dst + r].copy_from_slice(&data[pos..pos + r]);
            buf[dst + r..dst + 32].fill(0);
        }

        let mut out = [0u8; 32];
        let input: &[u8; 64] = (&buf[buf_ptr as usize..buf_ptr as usize + 64])
            .try_into()
            .unwrap();
        haraka512_5(&mut out, input);

        buf[buf_ptr2 as usize..buf_ptr2 as usize + 32].copy_from_slice(&out);
    ...
    result.copy_from_slice(&buf[buf_ptr as usize..buf_ptr as usize + 32]);
    result
}
...
mod tests {
    ...
    // Prevhash reversed
    header.extend(hex::decode("8555411fd8c46508a3a166ff52bc21fdb9d3633ae20ee4fee6aa257fdcf17bab").unwrap());

    // Merkleroot reversed
    header.extend(hex::decode("8b5c9a26578ded9888695876dcc669bbef31d26142a3f6222654be8abef3f82b").unwrap());

    // Finalsaplingroot als reserved reversed
    header.extend(hex::decode("2ca7fe83e5b594a36d546143e19a67325b569d7a9ffdba453d152144cab9b289").unwrap());
    ...
    header.extend(hex::decode("0000f4c2224281c37af556fa4396eceb8eb746332476c7442692a68a76ed17a5").unwrap());
